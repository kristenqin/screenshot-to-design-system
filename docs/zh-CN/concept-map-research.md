...
当前文档站通过 Cytoscape.js 复用已有图渲染能力，在外层补上 Obsidian 风格的交互控制，而不是重新自研渲染器。

重要修正：

- 下一轮拟真不应该只基于产品参考继续调参数。
- 项目已经单独调研开源实现，见 [Obsidian Graph 开源实现调研](obsidian-graph-open-source-research.md)。
- GraphFrontier 是下一轮高拟真 spike 的首要复用/参考候选。

Discovery gate:

- Problem：让知识图谱像 Obsidian Graph View 一样可阅读、可控制。
- Existing options checked：Obsidian Graph View 作为产品参考；Cytoscape.js 作为当前已采用渲染器；Sigma.js 作为未来大图候选。
- Recommended path：保留 Cytoscape.js，在其上增加 Obsidian 式控制。
- Why not self-build：力导向布局、pan/zoom、选择、邻域遍历已经由 Cytoscape 提供。
- Verification：浏览器验证 global/local、local depth、graph settings 和节点/边数量。
- Reversibility：这些控制都是前端状态，不改变 Markdown manifest。

已实现的交互控制：

- `Scope`：中文决策图、English execution、All audit。
- `View`：全局图或围绕当前文档的局部图。
- `Local depth`：从当前文档向外扩展 1、2、3 跳。
- `Graph settings`：labels、arrows、bilingual links、path links、node size、link thickness、repel force、link distance。

默认行为会让图更安静：

- 文档引用关系默认可见。
- 中英文 companion 关系默认隐藏，但可以打开。
- reading path 关系默认隐藏，但可以打开。
- 标签默认隐藏，需要时再打开。

推荐使用方式：

1. 先从 global graph 看整体结构。
2. 决策时切到中文图，执行时切到英文图。
3. 检查某个文档的影响范围时切到 local graph。
4. 邻域太窄时再增加 local depth。
5. 只有需要时才打开 labels、arrows、path links 或 bilingual links。
